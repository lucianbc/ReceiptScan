...
private fun determineFormat(dateString: String): String? {
    for ((regex, format) in formats) {
        if (dateString.matches(regex.toRegex())) {
            return format
        }
    }
    return null
...
fun parseDate(dateString: String): Date? {
    ...
    val format = determineFormat(dateString)
    return if (format != null) SimpleDateFormat(format, Locale.US).parse(lowerDate) else null
}


fun Date.show() = format.format(this)
